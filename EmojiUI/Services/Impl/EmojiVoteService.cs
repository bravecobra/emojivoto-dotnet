...
{
    public class EmojiVoteService : IEmojiVoteService
    {
        ...
        public EmojiVoteService(VotingService.VotingServiceClient votingClient, EmojiService.EmojiServiceClient emojiClient)
        {
        ...
        {
            var response = await _votingClient.ResultsAsync(new ResultsRequest());
            var result = new List<Result>();
            foreach (var responseResult in response.Results)
            {
                var emoji = await _emojiClient.FindByShortcodeAsync(new FindByShortcodeRequest
                {
                    Shortcode = responseResult.Shortcode
                });
                result.Add(new Result
                {
                    Shortcode = emoji.Emoji.Shortcode,
                    Unicode = emoji.Emoji.Unicode,
                    Votes = responseResult.Votes
                });
            }
